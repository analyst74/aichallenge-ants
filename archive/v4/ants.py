...
class Ants():
    ...
    def get_all_neighbours(self, loc):
        'get all neighbour locations, except those are water'
        return [self.destination(loc, direction) for direction in ALL_DIRECTIONS]
    ...
    def get_combat_zones_3(self):
        'get all my fighter ants in groups'
        group_distance = 3**2
        enemy_distance = self.euclidean_distance_add(self.attackradius2, 3)
        enemy_ants = [ant_loc for ant_loc, owner in self.enemy_ants()]
        open_fighters = self.bfs(enemy_ants, enemy_distance, 
            lambda loc : self.map[loc[0]][loc[1]] == MY_ANT and self.ant_list[loc][1] == False)
        ...
        while len(open_fighters) > 0:
            ...
            while len(group) > group_i:
                friends = [ant for ant in open_fighters if self.euclidean_distance2(group[group_i], ant) < group_distance]
                group.extend(friends)
                open_fighters = [ant for ant in open_fighters if ant not in friends]
                group_i += 1
            
            group_enemy = self.bfs(group, enemy_distance, lambda loc : loc in enemy_ants)
            fighter_groups.append((group, group_enemy))

        return fighter_groups
        
    def eval_formation_3(self, my_formation, enemy_formation):
        'return score, min_distance for the given my_formation/enemy_formation'
        # generate all pairs
        all_pairs = [(m, e) for m in my_formation for e in enemy_formation]
        # find the min_distance between our ants
        all_distances = [self.euclidean_distance2(m,e) for m in my_formation for e in enemy_formation]
        min_distance = self.euclidean_distance_add(min(all_distances), 0.5)
        # find out fighting pairs by getting all_pairs that has min_distance
        fighting_pairs = [all_pairs[i] for i,x in enumerate(all_distances) if x <= min_distance]
        # create set (this ensures uniqueness)
        my_fighters = {}
    ...
    def do_zone_combat_3(self, zone):
        ...
        score, target_distance = self.eval_formation_3(my_group, enemy_group)
        ...
        if score > 0:
            ...
            target_distance = self.euclidean_distance_add(target_distance, 1)
            
        # for each ant, figure out a position that is smallest above average
        for ant in my_group:
            possible_moves = [ant] + [n_loc for n_loc in self.get_all_neighbours(ant) 
                if self.passable(n_loc)]
            move_distances = []
            for move in possible_moves:
                me_pair = [(move,e) for e in enemy_group]
                me_distances = [self.euclidean_distance2(m,e) for (m,e) in me_pair]
            ...
            # order
            direction = self.direction(ant, best_move)
            self.issue_order((ant, None if len(direction) == 0 else direction[0]))
    
    def do_zone_combat_2(self, zone):
