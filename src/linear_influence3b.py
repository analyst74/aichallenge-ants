...
class LinearInfluence():
    ...
    def set_influence(self, influence_sources, abort_on_friendly):
        ...
        for i in range(len(influence_sources)):# initial_loc, initial_value in influence_sources:
        ...
        for i in range(np_temp_maps.shape[0]):
            for row in range(20,27):
                debug_logger.debug(np_temp_maps[i,row,54:60])
        for row in range(20,27):
            debug_logger.debug(self.map[row,54:60])
